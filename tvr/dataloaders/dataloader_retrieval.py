...
class RetrievalDataset(Dataset):
    ...
    def __init__(
        ...
        self.video_dict, self.sentences_dict, self.sentences_per_video_dict, self.vocab_dict = self._get_anns(self.subset)
    ...
    def _get_text(self, caption, video_id):
        if len(caption) == 3:
        ...
        if self.prompt_tuning:
            ...
            caption_text = text_aug.format(caption_text)
        ...
        return input_ids, input_mask, s, e, caption_text

    def _get_vocab_text(self, video_id):

        vocab_list = self.vocab_dict[video_id]
        selected_vocabs = vocab_list[0:1]
        selected_vocabs = ', '.join(selected_vocabs)

        text_aug = "This is a video about {}."
        caption_text = text_aug.format(selected_vocabs.strip().rstrip())
        words = self.tokenizer.tokenize(caption_text)
        ...
        return input_ids, input_mask, caption_text

    def _get_rawvideo(self, video_id, s=None, e=None):
        video_mask = np.zeros(self.max_frames, dtype=np.longlong)
        max_video_length = 0
    ...
    def _get_rawvideo_dec(self, video_id, s=None, e=None):
        ...
        # T x 3 x H x W
        video = np.zeros((self.max_frames, 3, self.image_resolution, self.image_resolution), dtype=np.float64)
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'all':
            ...
            text_ids, text_mask, s, e, text_caption = self._get_text(caption, video_id)
            video, video_mask = self._get_rawvideo_dec(video_id, s, e)

            text_vocab_ids, text_vocab_mask, vocab_caption = self._get_vocab_text(video_id)
            # video, video_mask = self._get_rawvideo(video_id, s, e)

            # print(f"{text_caption}\n{vocab_caption}\n")
            data = {"text_ids": text_ids, "text_mask": text_mask, "text_vocab_ids": text_vocab_ids, "text_vocab_mask": text_vocab_mask, 
                    "video": video, "video_mask": video_mask, "idx": idx}
        elif self.mode == 'text':
            video_id, caption = self.sentences_dict[idx]
            text_ids, text_mask, s, e = self._get_text(caption)
            data = {"text_ids": text_ids, "text_mask": text_mask, "idx": idx}
        elif self.mode == 'video':
            video_id = self.video_list[idx]
            video, video_mask = self._get_rawvideo_dec(video_id)
            text = self.sentences_per_video_dict[video_id]
            # video, video_mask = self._get_rawvideo(video_id)
            data = { "video": video, 
                    "video_mask": video_mask, "idx": idx, "video_id": video_id, "text": text}
        return data
