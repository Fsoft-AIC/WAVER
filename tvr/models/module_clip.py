...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, attn_mask=None):
        attn_mask_ = attn_mask if attn_mask is not None else self.attn_mask
        if attn_mask is None and self.attn_mask is not None and hasattr(self.attn_mask, '__call__'):
...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.input_resolution = input_resolution
        self.image_size = (input_resolution, input_resolution)
