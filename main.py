...
from tvr.models.modeling import OC_CLIP, AllGather
...
def get_args(description='Open-Vocabulary Text-Video Retrieval'):
    ...
    parser.add_argument('--top_k', type=int, default=1, help="select a architecture for weight branch")
    parser.add_argument("--mode", default="all", type=str, help="multi machine DDP")
    parser.add_argument("--prompt_tuning", default=False, action='store_true', help="multi machine DDP")
    parser.add_argument("--drop_last", default=False, action='store_true', help="multi machine DDP")
...
def prep_optimizer(args, model, num_train_optimization_steps, local_rank):
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in decay_clip_param_tp], 'params_name': [n for n, p in decay_clip_param_tp], 'weight_decay': weight_decay, 'lr': lr * coef_lr},
        {'params': [p for n, p in decay_noclip_param_tp], 'params_name': [n for n, p in decay_noclip_param_tp], 'weight_decay': weight_decay},
        {'params': [p for n, p in no_decay_clip_param_tp], 'params_name': [n for n, p in no_decay_clip_param_tp], 'weight_decay': 0.0, 'lr': lr * coef_lr},
        {'params': [p for n, p in no_decay_noclip_param_tp], 'params_name': [n for n, p in no_decay_noclip_param_tp], 'weight_decay': 0.0}
    ]
    ...
    for group in optimizer.param_groups:
        param_lr = group["lr"]
        param_wd = group["weight_decay"]
        for (param_name, param) in zip(group["params_name"], group["params"]):
            optim_info["Param Name"].append(param_name)
            optim_info["Param Shape"].append(
                "X".join([str(x) for x in list(param.shape)])
            )
            optim_info["Required Grad"].append(param.requires_grad)
            total_params_size += param.numel()
            if param.requires_grad == True:
                trainable_params_size += param.numel()
            optim_info["Lr"].append(param_lr)
            optim_info["Wd"].append(param_wd)
    # Counting the number of parameters
    optim_info["Param Name"].append("Total Model Params")
    optim_info["Param Shape"].append("{:.2f}M".format(total_params_size / 1e6))
    optim_info["Required Grad"].append("-")
    optim_info["Lr"].append("-")
    optim_info["Wd"].append("-")

    optim_info["Param Name"].append("Total Trainable Params")
    optim_info["Param Shape"].append("{:.2f}M".format(trainable_params_size / 1e6))
    optim_info["Required Grad"].append("-")
    optim_info["Lr"].append("-")
    optim_info["Wd"].append("-")
    table = tabulate(
...
def _run_on_single_gpu(model, t_mask_list, v_mask_list, vocab_mask_list, t_feat_list, v_feat_list, vocab_feat_list, mini_batch=32):
    sim_matrix = []
    ...
    batch_vocab_mask = torch.split(vocab_mask_list, mini_batch)
    batch_t_feat = torch.split(t_feat_list, mini_batch)
    batch_v_feat = torch.split(v_feat_list, mini_batch)
    batch_vocab_feat = torch.split(vocab_feat_list, mini_batch)
...
def eval_epoch(args, model, test_dataloader, device):
    ...
    # ----------------------------
    batch_mask_vocab, batch_mask_t, batch_mask_v, batch_feat_vocab, batch_feat_t, batch_feat_v, ids_vocab, ids_t, ids_v = [], [], [], [], [], [], [], [], []
    ...
    with torch.no_grad():
        ...
        if multi_sentence_:  # multi-sentences retrieval means: one clip has two or more descriptions.
            ...
            for data in tqdm(test_dataloader[0]):
                text_ids, text_mask, inds = data["text_ids"].to(device), data["text_mask"].to(device), data["idx"].to(device)
            ...
            for data in tqdm(test_dataloader[1]):
                video, video_mask, inds = data["video"].to(device), data["video_mask"].to(device), data["idx"].to(device)
            ...
            for data in tqdm(test_dataloader):
                ...
                video, video_mask, inds = data["text_ids"].to(device), data["text_mask"].to(device), \
                        data["text_vocab_ids"].to(device), data["text_vocab_mask"].to(device), \
                        data["video"].to(device), data["video_mask"].to(device), data["idx"].to(device)
                text_feat, text_vocab_feat, video_feat = model.get_text_video_feat(text_ids, text_mask, text_vocab_ids, text_vocab_mask, video, video_mask)
                ids_t.append(inds)
                batch_mask_t.append(text_mask)
                batch_mask_v.append(video_mask)
                batch_mask_vocab.append(text_vocab_mask)
                batch_feat_t.append(text_feat)
                batch_feat_v.append(video_feat)
                batch_feat_vocab.append(text_vocab_feat)
            ids_t = allgather(torch.cat(ids_t, dim=0), args).squeeze()
            batch_mask_t = allgather(torch.cat(batch_mask_t, dim=0), args)
            batch_mask_v = allgather(torch.cat(batch_mask_v, dim=0), args)
            batch_mask_vocab = allgather(torch.cat(batch_mask_vocab, dim=0), args)
            batch_feat_t = allgather(torch.cat(batch_feat_t, dim=0), args)
            batch_feat_v = allgather(torch.cat(batch_feat_v, dim=0), args)
            batch_feat_vocab = allgather(torch.cat(batch_feat_vocab, dim=0), args)
            batch_mask_t[ids_t] = batch_mask_t.clone()
            batch_mask_v[ids_t] = batch_mask_v.clone()
            batch_mask_vocab[ids_t] = batch_mask_vocab.clone()
            batch_feat_t[ids_t] = batch_feat_t.clone()
            batch_feat_v[ids_t] = batch_feat_v.clone()
            batch_feat_vocab[ids_t] = batch_feat_vocab.clone()
            batch_mask_t = batch_mask_t[:ids_t.max() + 1, ...]
            batch_mask_v = batch_mask_v[:ids_t.max() + 1, ...]
            batch_mask_vocab = batch_mask_vocab[:ids_t.max() + 1, ...]
            batch_feat_t = batch_feat_t[:ids_t.max() + 1, ...]
            batch_feat_v = batch_feat_v[:ids_t.max() + 1, ...]
            batch_feat_vocab = batch_feat_vocab[:ids_t.max() + 1, ...]
            logger.info('[finish] extract text+video feature')

    toc1 = time.time()

    logger.info('{} {} {} {} {} {}'.format(len(batch_mask_t), len(batch_mask_v), len(batch_mask_vocab), len(batch_feat_t), len(batch_feat_v), len(batch_feat_vocab)))
    # ----------------------------------
    # 2. calculate the similarity
    # ----------------------------------
    logger.info('[start] calculate the similarity')
    with torch.no_grad():
        sim_matrix = _run_on_single_gpu(model, batch_mask_t, batch_mask_v, batch_mask_vocab, batch_feat_t, batch_feat_v, batch_feat_vocab)
